...
class ZHPageViewController: ZHBaseViewController {
    ...
    func gotoIndex(index:Int){
        if currentSelectIndex != index {
            var realIndex = index
            if index >= vcs.count {
                realIndex = vcs.count - 1
            }
            let target:[UIViewController] = [vcs[realIndex]]
            let direction:UIPageViewControllerNavigationDirection = currentSelectIndex > realIndex ? .reverse : .forward
            pageVC.setViewControllers(target, direction: direction, animated: false) { [weak self] (finish) in
                self?.currentSelectIndex = realIndex
                self?.segment.setSelectedSegmentIndex(UInt(realIndex), animated: false)
            }
        }
    }
    override func configUI() {
        ...
        case .navgationBarSegment?:
            ...
            pageVC.view.snp.makeConstraints { $0.edges.equalToSuperview() }
        case .topTabBar?:
            segment.titleTextAttributes = [NSAttributedStringKey.foregroundColor: UIColor.subTitleColor,
                                           NSAttributedStringKey.font: UIFont.medium15]
            segment.selectedTitleTextAttributes = [NSAttributedStringKey.foregroundColor: UIColor.titleColor,
                                                   NSAttributedStringKey.font: UIFont.medium15]
            segment.selectionIndicatorLocation = .down
            segment.selectionIndicatorColor = UIColor.deadlineColor
            ...
            let img = UIImageView(image:UIImage(named:"mine_shadow"))
...
extension ZHPageViewController: UIPageViewControllerDataSource, UIPageViewControllerDelegate {
